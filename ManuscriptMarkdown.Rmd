...
Packages <- c("ggplot2","dplyr","reshape","wesanderson",
              ...
              "cowplot","readr") # Slew of packages used here.
...
PainCodes <- read.csv("PainRatings.csv")
PainCodes$Subjidx <- paste("CLP",PainCodes$Subjidx,sep = "")

Subjidx <- dir(path = "Data/",
               pattern = "CLP*")

# Remove CLP116 - Didn't finish
Subjidx <- Subjidx[Subjidx != "CLP116"]
# Remove CLP1107 - Odd Trigger codes 
Subjidx <- Subjidx[Subjidx != "CLP107"]
# Remove CLP130 - Odd trigger codes
Subjidx <- Subjidx[Subjidx != "CLP130"]
...
Trial.list <- list(np = list(),
                   p = list())
## Extract Trial.list
for(px in seq_along(Subjidx)){
  Trial.list[["np"]][[px]] <- ExtractTrialData(PartID = Subjidx[px],
                                               COND = "np",
                                               dataloc = "Data/")
  
}
names(Trial.list[["np"]]) <- Subjidx
Trial.list[["np"]] <- Trial.list[["np"]][!is.na(Trial.list[["np"]])]

for(px in seq_along(Subjidx)){
  Trial.list[["p"]][[px]] <- ExtractTrialData(PartID = Subjidx[px],
                                               COND = "p",
                                               dataloc = "Data/")
  
}
names(Trial.list[["p"]]) <- Subjidx
Trial.list[["p"]] <- Trial.list[["p"]][!is.na(Trial.list[["p"]])]
...
# Mark participants who recieved pain:
Trialdata$Rec_pain <- 0
Trialdata$Rec_pain[Trialdata$PartID %in% PainCodes$Subjidx] <- 1
...
# Subset Participants from Trialdata
Trialdata <- Trialdata[Trialdata$PartID %in% Subjidx,]
Trialdata$PartID <- factor(Trialdata$PartID)
rownames(Trialdata) <- seq(from = 1, to = nrow(Trialdata),by=1)
```

``` {r Sanity Checks}

#### TODO Fix this section up. Make it presentable ####
### We want to:
# Find incorrect trigger codes, i.e., short/long CTI/ITIs and remove
# Cut short RT ~<150ms
# Cut long RT >1000ms?


hist(Trialdata$RT, breaks = 100)
hist(Trialdata$RT[Trialdata$RT<250], breaks = 100)

table(Trialdata$PartID[Trialdata$RT<150])
table(Trialdata$Condition[Trialdata$RT<150]) # Slight condition difference
table(Trialdata$Accuracy[Trialdata$RT<150])

# Long responses
table(Trialdata$PartID[Trialdata$RT>800])
table(Trialdata$Condition[Trialdata$RT>800])
table(Trialdata$Accuracy[Trialdata$RT>800])

## check CTI and ITI values:
hist(Trialdata$CTI, breaks = 100)
hist(Trialdata$CTI[Trialdata$CTI<700], breaks = 100)
table(Trialdata$PartID[Trialdata$CTI<600]) # One short CIT. - Error
ShortCTI <- list(,
                 )


# Remove it
Trialdata <- Trialdata[-c(10271,12882),]

hist(Trialdata$CTI[Trialdata$CTI>800], breaks = 100)
table(Trialdata$PartID[Trialdata$CTI>850]) # One long CTI - Miss



median(Trialdata$CTI, na.rm = T)
min(Trialdata$CTI[Trialdata$CTI>500], na.rm = T)


hist(Trialdata$ITI, breaks = 100)
table(Trialdata$ITI)
hist(Trialdata$ITI[Trialdata$ITI>2500], breaks = 100)
hist(Trialdata$ITI[Trialdata$ITI<2000], breaks = 100)

table(Trialdata$PartID[Trialdata$ITI>3000])
table(Trialdata$PartID[Trialdata$ITI<1850])

Trialdata <- Trialdata[-c(4823),]


summary(Trialdata$ITI[Trialdata$ITI<3000])

Trialdata$RT[Trialdata$RT<150] <- NA

## Re-level the CueType data into cued and ambiguous
levels(Trialdata$CueType) <- list("Ambiguous" = "Ambiguous", "Cued" = c("Auditory","Visual"))


## Plot pain response
painplots <- PainCodes %>% dplyr::filter(AveragePainRating >= 20) %>% dplyr::select(Subjidx,Start,contains("Break")) %>% 
  tidyr::gather(Time,Pain,Start:Break5)
painplots$Time <- factor(painplots$Time,levels = c("Start", "Break1", "Break2", "Break3", "Break4", "Break5"))

painplots_summ <- Rmisc::summarySEwithin(
  painplots,
  measurevar = "Pain",
  withinvars = "Time",
  idvar = "Subjidx",
...
ggplot(data = painplots,aes(x = Time,y = Pain,group = Subjidx,colour = Subjidx)) +
  geom_point() + geom_line() +
    stat_summary(aes(y = Pain,group=1), fun=mean, colour="red", lwd = 1, geom="line",group=1) +
      # stat_summary(aes(y = Pain,group=1),fun.data = mean_se,colour="red", lwd = 1,
      #            geom = "errorbar")+
  theme_classic() +
...
# Mark participants who recieved pain:
Summdata$Rec_pain <- 0
Summdata$Rec_pain[Summdata$PartID %in% PainCodes$Subjidx] <- 1
```
...
The Cross-modal Attention task requires participants to respond to either the position of a visual target or the pitch of an auditory target. Targets can appear simultaneously and so participants are presented with a cue to indicate the correct target in the upcoming trial. This version of the task consisted of three types of cues. Cues consisted of *Insert what cues looked like* presented on screen for *How long were cues presented?*. Cue *one* indicated the participant should respond to the position of the visual target. Cue *two* indicated that participants should respond to the pitch of the auditory target. These two cues occurred on `r round((1-(table(Trialdata$CueType)[[1]]/sum(table(Trialdata$CueType))))/2,1)*100`% of trials each. On the remaining `r round((table(Trialdata$CueType)[[1]]/sum(table(Trialdata$CueType))),1)*100`% of trials, participants received an ambiguous cue, which gave no information about the upcoming target. On these trials only one target was presented. Equal ratios of auditory and visual targets followed ambiguous cues (*I'm guessing this is true?*).
  ...
  Cues were displayed for 250ms and targets for 60ms [*Please confirm this is correct. Trigger codes did not specify exactly*]. The cue to target interval (CIT) jittered between `r min(Trialdata$CTI[Trialdata$CTI>300],na.rm = T)`-`r max(Trialdata$CTI[Trialdata$CTI<1400],na.rm = T)` in steps of **XXX** (*Values for this change with participants. Some have maxes of ~800ms and min of ~700ms. Others have max ~1500 and min ~200*). Each trial lasted an average of `r round(mean(Trialdata %>% group_by(PartID) %>% summarise(meanITI = mean(ITI,na.rm=T)) %>% .$meanITI),0)`ms.
...
EEGPreProc[["BadChan"]][["p"]] <- lapply(Subjidx,
                                         function(px)
                                           nrow(
                                             read.table(
                                               paste("Data/",
...
names(EEGPreProc[["BadChan"]][["p"]]) <- Subjidx
...
EEGPreProc[["RejComp"]][["p"]] <- lapply(Subjidx,
                                         function(px)
                                           nrow(
                                             read.table(
                                                paste("Data/",
...
names(EEGPreProc[["RejComp"]][["p"]]) <- Subjidx
...
## Summarise number of removed trials - Removed from current pipeline
...
Plots <- list()
RT_comp_xvals <- c("Target","CueType","Distractor")
RT_comp_groupvals <- c("Accuracy","Target","CueType","Distractor")
...
Plots[[RT_comp_xvals[i]]] <- lapply(RT_comp_groupvals,function(x) RT_RainPlot(data = Trialdata[Trialdata$Rec_pain==1,],
                                                                      xval = "Condition",
                                                                      groupval = x))
names(Plots[[RT_comp_xvals[i]]]) <- RT_comp_groupvals
...
Error_comp_groupvals <- c("Target","CueType","Distractor")
...
Err_Plots[["Condition"]] <- lapply(Error_comp_groupvals, function(x)
  Err_RainPlot(
  data = N_Trials[[x]][N_Trials[[x]]$PartID %in% PainCodes$Subjidx,],
  xval = "Accuracy",
  groupval = x
  ))
...
# Errors
totalErrors <- Trialdata %>% dplyr::group_by(PartID, Condition, CueType, Target, Distractor, Accuracy) %>% dplyr::tally() %>% dplyr::group_by(PartID, Condition, CueType, Target, Distractor) %>% dplyr::mutate(total = sum(n)) %>% dplyr::mutate(perc = (n/total)*100) %>% dplyr::select(-n,-total) %>% dplyr::filter(Accuracy!="Correct")
...
# Write to file
write.csv(ProgramdataErr, file = "Output/Wide_Errdata.csv",
          row.names = F)

```

## 3.1. Behavioural Results
Table 1 summarises the behavioural data of the C-MAT task used here in both the pain and no pain conditions. Below is the analysis broken down by target modality, cue type, and distractor presence. 
...
Tab1_RT <- dplyr::bind_rows(RT_means$Correct,
                      .id = "Effect")
Tab1_RT$RT <- paste(round(Tab1_RT$RT,2)," (",round(Tab1_RT$SD,2),")",sep="")
Tab1_RT$SD <- NULL
Tab1_RT <- Tab1_RT %>% tidyr::spread(Condition,RT)

Tab1_RTerr <- dplyr::bind_rows(RT_means$Error,
                      .id = "Effect")
Tab1_RTerr$`Error RT` <- paste(round(Tab1_RTerr$`Error RT`,2)," (",round(Tab1_RTerr$SD,2),")",sep="")
Tab1_RTerr$SD <- NULL
Tab1_RTerr <- Tab1_RTerr %>% tidyr::spread(Condition,`Error RT`)

Tab1_Err <- dplyr::bind_rows(Error_means$Error,
                      .id = "Effect")
Tab1_Err$`Errors (%)` <- paste(round(Tab1_Err$`Errors (%)`,2)," (",round(Tab1_Err$SD,2),")",sep="")
Tab1_Err$SD <- NULL
Tab1_Err <- Tab1_Err %>% tidyr::spread(Condition,`Errors (%)`)

Tab1_Miss <- dplyr::bind_rows(Error_means$Miss,
                      .id = "Effect")
Tab1_Miss$`Omissions (%)` <- paste(round(Tab1_Miss$`Omissions (%)`,2)," (",round(Tab1_Miss$SD,2),")",sep="")
Tab1_Miss$SD <- NULL
Tab1_Miss <- Tab1_Miss %>% tidyr::spread(Condition,`Omissions (%)`)


## Bring tables together into table, dropping the effect and variable columns

Table1 <- cbind(Tab1_RT %>% select(-Effect),
                Tab1_RTerr %>% select(-Effect,-Variable),
                Tab1_Err %>% select(-Effect,-Variable),
                Tab1_Miss %>% select(-Effect,-Variable))

names(Table1)[names(Table1) == 'Variable'] <- ' '

# Print Table 1
# Column headers for Condition
# Row Groupings for Modality, Cue Type, Distractor
kable(Table1, caption = "Table 1. Summary of the behavioural data for the no pain and pain conditions, split by the cue type, distractor presence, and target modality.") %>%
  kable_styling("striped", full_width = F) %>%
 add_header_above(c(" ", "Correct RT (ms)" = 2, "Error RT (ms)" = 2, "Errors (%)" = 2, "Omissions (%)" = 2)) %>% 
  pack_rows("Cue Type", 1, 2) %>%
  pack_rows("Distractor", 3, 4) %>% 
  pack_rows("Modality", 5, 6) %>%
column_spec(1, bold = T, border_right = T) %>% 
  footnote(general = "Data are presented as the mean (standard deviation)."
)
...
```{r ANOVA}
## Run ANOVAs on each effect; Preparation, Distraction, Modality
...
## Full model
ANOVAtest[["Full"]] <- BayesFactor::lmBF(
    formula = RT ~ Target * CueType * Distractor * Condition + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
      dplyr::group_by(PartID, Target, CueType, Distractor, Condition) %>%
      dplyr::summarise(RT = mean(RT, na.rm = T)),
    whichRandom = "PartID"
  )

ANOVAtest[["MainEff"]] <- BayesFactor::lmBF(
    formula = RT ~ Target + CueType + Distractor + Condition + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
...
# Support for just the main effects compared to the interactions
ANOVAtest[["MainEff"]]/ANOVAtest[["MainEff"]]

## Now build models for each main effect to compare against 
ANOVAtest[["Null"]] <- BayesFactor::lmBF(
    formula = RT ~ 1 + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
...
ANOVAtest[["Condition"]] <- BayesFactor::lmBF(
    formula = RT ~ Condition + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
      dplyr::group_by(PartID, Target, CueType, Distractor, Condition) %>%
...
ANOVAtest[["Target"]] <- BayesFactor::lmBF(
    formula = RT ~ Target + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
      dplyr::group_by(PartID, Target, CueType, Distractor, Condition) %>%
...
ANOVAtest[["CueType"]] <- BayesFactor::lmBF(
    formula = RT ~ CueType + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
      dplyr::group_by(PartID, Target, CueType, Distractor, Condition) %>%
...
ANOVAtest[["Distractor"]] <- BayesFactor::lmBF(
    formula = RT ~ Distractor + PartID,
    data = Trialdata %>%
      dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
      dplyr::group_by(PartID, Target, CueType, Distractor, Condition) %>%
...
incmods <- bayestestR::bayesfactor_models(
  ANOVAtest[["Condition"]],ANOVAtest[["Target"]],ANOVAtest[["CueType"]],ANOVAtest[["Distractor"]],ANOVAtest$MainEff,ANOVAtest$Full,
  denominator =  ANOVAtest[["Null"]])
...
InteractionTests[["Full"]][["Inclusion"]] <- bayestestR::bayesfactor_inclusion(
  InteractionTests[["Full"]][["ANOVA"]],
  match_models = T
)

## Error Test
full <- BayesFactor::anovaBF(
  formula = perc ~ Target * CueType * Distractor * Condition + PartID,
  data = Trialdata %>%
    dplyr::group_by(PartID, Condition, CueType, Target, Distractor, Accuracy) %>%
    dplyr::tally() %>%
    dplyr::group_by(PartID, Condition, CueType, Target, Distractor) %>%
    dplyr::mutate(total = sum(n)) %>%
    dplyr::mutate(perc = (n / total) * 100) %>%
    dplyr::select(-n,-total) %>%
    dplyr::filter(Accuracy == "Error") %>%
...
# papaja::apa_print.BFBayesFactor(InteractionTests[["Full"]][["ANOVA"]])

## Follow-up Bayes t-tests
BayesT <- list(Modality = list(),
  Preparation = list(),
  Distractor = list(),
  Condition = list()
)
...
BayesT[["Preparation"]] <- 
    BayesFactor::ttestBF(
      x = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                      Rec_pain == 1) %>%
        dplyr::group_by(PartID, CueType) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(CueType, RT) %>% .$Ambiguous,
      y = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, CueType) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(CueType, RT) %>% .$Cued,
      paired = T)
...
BayesT[["Modality"]] <-
    BayesFactor::ttestBF(
      x = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, Target) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Target, RT) %>% .$Visual,
      y = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, Target) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Target, RT) %>% .$Auditory,
      paired = T)
...
BayesT[["Distractor"]] <-
    BayesFactor::ttestBF(
      x = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, Distractor) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Distractor, RT) %>% .$Absent,
      y = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, Distractor) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Distractor, RT) %>% .$Present,
      paired = T)
...
BayesT[["Condition"]] <-
    BayesFactor::ttestBF(
      x = Trialdata %>%
        ...
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Condition, RT) %>% .$`No Pain`,
      y = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                    Rec_pain == 1) %>%
        dplyr::group_by(PartID, Condition) %>%
        dplyr::summarise(RT = mean(RT, na.rm = T)) %>%
        tidyr::spread(Condition, RT) %>% .$Pain,
      paired = T)

## Post-hoc t-test to confirm effect of pain
BayesT[["Modality"]][["Pain"]] <-
  sapply(c("Visual", "Auditory"), function(i)
    BayesFactor::ttestBF(
      x = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                      Target == i,
...
BayesT[["Distractor"]] <-
  sapply(c("No Pain", "Pain"), function(i)
    BayesFactor::ttestBF(
      x = Trialdata %>%
        dplyr::filter(Accuracy == "Correct",
                      Condition == i,
...
```
  ...
  In the full ANOVA model there was evidence to support each main effect, seperately. 
...
Plotdata[["PreparationCOND"]] <- Trialdata %>%
  dplyr::filter(Accuracy == "Correct",
                Rec_pain == 0) %>% # This is necessary across all RT data - Hard coded!
  dplyr::group_by(PartID, CueType, Condition) %>%
  dplyr::summarise(RT = mean(RT, na.rm = T))

Plotdata[["DistractionCOND"]] <- Trialdata %>%
  dplyr::filter(Accuracy == "Correct",
                Rec_pain == 0,
                CueType == "Cued",
                Target == "Visual") %>% 
  dplyr::group_by(PartID, Distractor, Condition) %>%
  dplyr::summarise(RT = mean(RT, na.rm = T))


wse <- list()
wse[["PreparationCOND"]] <- Rmisc::summarySEwithin(
  Plotdata[["PreparationCOND"]],
  measurevar = "RT",
...
wse[["DistractionCOND"]] <- Rmisc::summarySEwithin(
  Plotdata[["DistractionCOND"]],
  measurevar = "RT",
...
# ## Make and save plot
# ggplot(Plotdata[["PreparationCOND"]], aes(x = CueType, y = RT, fill = Condition)) +
#       geom_boxplot(
#       aes(x = CueType, y = RT, fill = Condition),
#       outlier.shape = NA,
#       alpha = .5,
#       width = .1,
#       colour = "black"
...
#       size = 3
#     ) +
#   geom_line(data = wse[["PreparationCOND"]],
#       aes(x = as.numeric(CueType) -.15,
#         y = RT,
#         group = Condition,
#         colour = Condition))+
#     geom_errorbar(
...
#     ggsave(
#       paste("Output/Plots/PreparationCOND.tiff", sep = ""),
#       width = w,
#       height = h)

ggplot(Plotdata[["DistractionCOND"]], aes(x = Distractor, y = RT, fill = Condition)) +
  geom_boxplot(
    aes(x = Distractor, y = RT, fill = Condition),
    outlier.shape = NA,
    alpha = .5,
    width = .1,
    colour = "black"
  ...
  geom_line(data = wse[["DistractionCOND"]],
            aes(
              x = as.numeric(Distractor) - .15,
              y = RT,
              group = Condition,
  ...
  theme_classic() +
  theme(legend.position = c(.9, .2)) +
  ggsave("Output/Plots/DistractionCOND.png")
...
BayesT[["CueTypeCondition"]] <- Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)) %>% 
    tidyr::unite(Effect,Condition,CueType) %>% 
    tidyr::spread(Effect,RT)

write.csv(BayesT$CueTypeCondition,
          file = "Output/CueTypeCond.csv")

BayesT[["TargetCondition"]] <- Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, Target, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)) %>% 
    tidyr::unite(Effect,Condition,Target) %>% 
    tidyr::spread(Effect,RT)

write.csv(BayesT$TargetCondition,
          file = "Output/TargetCond.csv")

BayesT[["DistractorCondition"]] <- Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)) %>% 
    tidyr::unite(Effect,Condition,Distractor) %>% 
    tidyr::spread(Effect,RT)

write.csv(BayesT$DistractorCondition,
          file = "Output/DistractorCond.csv")

ProgramData <- dplyr::bind_cols(BayesT)

# No pain Preparation (CueType) Effect
ProgramData$np_PreparationEff <- ProgramData$`No Pain_Ambiguous`- ProgramData$`No Pain_Cued`

# No pain Target Effect
ProgramData$np_TargetEff <- ProgramData$`No Pain_Auditory`- ProgramData$`No Pain_Visual`

# No pain Distraction Effect
ProgramData$np_DistractionEff <- ProgramData$`No Pain_Present`- ProgramData$`No Pain_Absent`

# Pain Preparation (CueType) Effect
ProgramData$p_PreparationEff <- ProgramData$Pain_Ambiguous- ProgramData$Pain_Cued

# Pain Target Effect
ProgramData$p_TargetEff <- ProgramData$Pain_Auditory- ProgramData$Pain_Visual

# Pain Distraction Effect
ProgramData$p_DistractionEff <- ProgramData$Pain_Present- ProgramData$Pain_Absent

write.csv(ProgramData,
          file = "Output/TtestData.csv")
...
BayesModels[["CueType"]] <-
  BayesFactor::lmBF(
    ...
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
    whichRandom = "PartID",
    progress = F,
    iterations = 10000,posterior = F)
BayesModels[["CueTypeCondition"]] <-
  BayesFactor::lmBF(
    RT ~ CueType + Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
...
BayesModels[["CueTypeConditionInt"]] <-
  BayesFactor::lmBF(
    RT ~ CueType * Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
...
BayesModels[["TargetCondition"]] <-
  BayesFactor::lmBF(
    RT ~ Target + Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
...
BayesModels[["TargetConditionInt"]] <-
  BayesFactor::lmBF(
    RT ~ Target * Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
...
## Distractor
BayesModels[["Distractor"]] <-
  BayesFactor::lmBF(
    RT ~ Distractor  + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
...
BayesModels[["DistractorCondition"]] <-
  BayesFactor::lmBF(
    RT ~ Distractor + Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
...
BayesModels[["DistractorConditionInt"]] <-
  BayesFactor::lmBF(
    RT ~ Distractor * Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
...
BayesModels[["Full"]] <-
  BayesFactor::lmBF(
    RT ~ CueType + Target + Distractor + Condition + PartID,
    data = Trialdata %>%
    dplyr::filter(Accuracy == "Correct") %>%
    dplyr::group_by(PartID, CueType, Target, Distractor, Condition) %>%
    dplyr::summarise(RT = mean(RT, na.rm = T)),
...
# Compare models - Build them up some simple effects, to combined, to interaction
mods <- bayestestR::bayesfactor_models(BayesModels$CueType,
                                       BayesModels$Condition,
                                       BayesModels$Target,
                                       BayesModels$Distractor)
# BF inclusion
bayestestR::bayesfactor_inclusion(mods,match_models = T)
```

The effect of preparation on correct RT was assessed with a Bayesian two-way ANOVA of Cue Type (Ambiguous vs Cued) and Condition (No Pain vs Pain). Results suggested that there was evidence in favour of the effect of cueing ($BF$~10~ $=$ `r extractBF(BayesModels[["CueType"]],onlybf = T,logbf = F)`).  


Visual cues provided the fastest responses, faster than both Auditory ($BF$~10~ $=$ `r extractBF(Bayesposthoc[["VisAud"]],onlybf = T,logbf = F)`) and Ambiguous ($BF$~10~ $=$ `r extractBF(Bayesposthoc[["VisAmb"]],onlybf = T,logbf = F)`). Interestingly, Ambiguous cues were also found to be faster than 
followed by Ambiguous cues, and finally Auditory cues. This order was consistient across pain conditions, where RT remained relatively equal. 
